...
const STAGES = [
  { id: "snapshot",    label: "Account Snapshot",        icon: "🏢", color: "#3B82F6" },
  { id: "entity",      label: "AI / Tech Entity",         icon: "🤖", color: "#8B5CF6" },
  { id: "stack",       label: "Technology Stack",         icon: "⚙️",  color: "#06B6D4" },
  { id: "stakeholders",label: "Buying Group",             icon: "👥", color: "#10B981" },
  { id: "signals",     label: "Buying Signals",           icon: "📡", color: "#F59E0B" },
  { id: "pains",       label: "Business Pains & AWS",     icon: "🎯", color: "#EF4444" },
  { id: "competitive", label: "Competitive Landscape",    icon: "⚔️",  color: "#EC4899" },
  { id: "action",      label: "3-Week Action Plan",       icon: "🚀", color: "#F97316" },
  { id: "outreach",    label: "Outreach Emails",          icon: "✉️",  color: "#14B8A6" },
];

const SYSTEM_PROMPT = `You are an elite B2B account intelligence researcher for Younion × AWS. 
You build Account Intelligence Profiles (AIPs) for enterprise sales teams.
Your research is precise, sourced, and actionable. You write in clear human language — not jargon.
Every claim must be based on verifiable information. Flag uncertainty explicitly.
Format all responses as clean JSON only — no markdown, no preamble, no explanation outside the JSON.`;
...
Return this exact JSON structure (fill every field with real researched data):
{
  ...
  "digitalMaturity": "HIGH/MEDIUM/LOW with evidence — cloud usage, tech team, digital initiatives",
...
Return ONLY valid JSON:
{
  "stack": [
    {
      ...
      "depth": "DEEP/MODERATE/LIGHT — how embedded is this vendor",
  ...
  "keyPartners": "Main SIs — TCS, Infosys, Wipro etc",
  ...
  "stakeholders": [
    {
      ...
      "name": "Full name and title — use 'Not identified' if unknown",
...
Return ONLY valid JSON:
{
  "pains": [
    {
      ...
      "awsAnswer": "Specific AWS services and what outcome they deliver for this exact pain. Be concrete — name the services.",
  ...
  competitive: (company, url) => `Map the competitive technology landscape at ${company} (${url}) — who is already embedded, what they do, and how AWS should respond.
Return ONLY valid JSON:
{
  "competitors": [
    {
      ...
      "ourPlay": "Specific AWS response strategy — displace / co-exist / co-sell / extend",
      "source": "url"
    }
  ],
...
Return ONLY valid JSON:
{
  "emails": [
    {
      ...
      "body": "Full email body — 150-200 words. Reference specific company facts. Do not use generic AI language. Sound like a thoughtful human who has done their homework.",
  ...
  "linkedInMessages": [
    {
      "target": "Role name",
      "message": "LinkedIn connection message — max 300 characters. Specific, not generic."
    }
  ],
...
const PriorityBadge = ({ priority }) => {
  const cfg = {
    RED:      { bg: "#FEE2E2", color: "#991B1B", dot: "#EF4444", label: "🔴 Act Now" },
    ORANGE:   { bg: "#FEF3C7", color: "#92400E", dot: "#F59E0B", label: "🟠 This Month" },
    YELLOW:   { bg: "#FEF9C3", color: "#713F12", dot: "#EAB308", label: "🟡 Monitor" },
  ...
  return (
    <div style={{ border: `2px solid ${c}30`, borderLeft: `4px solid ${c}`, borderRadius: 10, padding: "14px 18px", marginBottom: 10, background: bg }}>
      ...
      <div style={{ background: "#FFFFFF80", border: `1px solid ${c}30`, borderRadius: 6, padding: "8px 12px", fontSize: 12 }}>
        <span style={{ fontWeight: 700, color: c }}>→ Action: </span>
        <span style={{ color: "#1E293B" }}>{signal.actionRequired}</span>
      </div>
      {signal.source && (
        <div style={{ marginTop: 6, fontSize: 11, color: "#94A3B8" }}>📎 {signal.source}</div>
      )}
    </div>
  );
};
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
      <div style={{ padding: "16px 18px", borderRight: "1px solid #E2E8F0" }}>
        ...
        {pain.revenueImpact && pain.revenueImpact !== "Not found" && (
          <div style={{ marginTop: 10, fontSize: 12, color: "#6B7280", fontStyle: "italic" }}>💰 {pain.revenueImpact}</div>
        )}
      </div>
      <div style={{ padding: "16px 18px", background: "#F0FDF4" }}>
        ...
        {pain.source && (
          <div style={{ marginTop: 10, fontSize: 11, color: "#94A3B8" }}>📎 {pain.source}</div>
        )}
      </div>
    </div>
  </div>
...
function RenderResults({ stageId, data }) {
  if (!data || data.error) return <div style={{ color: "#EF4444", fontSize: 13 }}>⚠️ {data?.error || "No data available"}</div>;
  ...
  switch (stageId) {
    case "snapshot":
      return (
        <div>
          ...
          {data.recentStructure && data.recentStructure !== "None found" && <Card style={{ borderLeft: "4px solid #F59E0B", background: "#FFFBEB" }}><div style={{ fontSize: 12, color: "#92400E", fontWeight: 700, marginBottom: 6 }}>⚡ RECENT RESTRUCTURING</div><div style={{ fontSize: 13, color: "#374151" }}>{data.recentStructure}</div></Card>}
          ...
          {data.keyFacts?.length > 0 && <Card><div style={{ fontSize: 12, color: "#94A3B8", fontWeight: 700, marginBottom: 8 }}>KEY FACTS</div>{data.keyFacts.map((f, i) => <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}><span style={{ color: "#3B82F6", fontWeight: 700 }}>→</span><span style={{ fontSize: 13, color: "#374151" }}>{f}</span></div>)}</Card>}
        </div>
      );
    ...
    case "entity":
      ...
      return (
        <div>
          ...
          {[["CEO", data.ceo], ["CTO", data.cto], ["CIN", data.cin], ["Capital & Status", data.capitalStatus], ["Core Focus", data.coreFocus?.join(" · ")], ["Vendor Deals on Record", data.vendorDealsOnRecord]].map(([k, v]) => v && (
          ...
          {data.urgencyNote && <Card style={{ marginTop: 14, background: "#FEF2F2", borderLeft: "4px solid #EF4444" }}><div style={{ fontSize: 12, color: "#991B1B", fontWeight: 700, marginBottom: 4 }}>⚡ URGENCY NOTE</div><div style={{ fontSize: 13, color: "#374151" }}>{data.urgencyNote}</div></Card>}
        </div>
      );
      ...
      return (
        <div>
          <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "12px 16px", marginBottom: 14 }}>
            ...
            {data.cloudDepth && <><span style={{ color: "#94A3B8", margin: "0 8px" }}>·</span><span style={{ color: "#475569", fontSize: 13 }}>{data.cloudDepth}</span></>}
          </div>
          ...
          {data.openOpportunities && <Card style={{ background: "#F0FDF4", borderLeft: "4px solid #10B981" }}><div style={{ fontSize: 12, color: "#065F46", fontWeight: 700, marginBottom: 6 }}>🎯 OPEN OPPORTUNITIES</div><div style={{ fontSize: 13, color: "#374151" }}>{data.openOpportunities}</div></Card>}
        </div>
      );
    ...
    case "stakeholders":
      return (
        <div>
          {data.entryPoint && <Card style={{ background: "#FFFBEB", borderLeft: "4px solid #F59E0B" }}><div style={{ fontSize: 12, color: "#92400E", fontWeight: 700, marginBottom: 4 }}>🎯 ENTRY POINT</div><div style={{ fontSize: 13, color: "#374151" }}>{data.entryPoint}</div></Card>}
          {data.stakeholders?.map((s, i) => (
            <Card key={i}>
              ...
              {s.linkedIn && s.linkedIn !== "Not found" && <div style={{ marginTop: 8, fontSize: 12, color: "#3B82F6" }}>🔗 {s.linkedIn}</div>}
            </Card>
          ))}
          {data.warmIntroPath && <Card style={{ background: "#EFF6FF" }}><div style={{ fontSize: 12, color: "#1E40AF", fontWeight: 700, marginBottom: 4 }}>🤝 WARM INTRO PATH</div><div style={{ fontSize: 13, color: "#374151" }}>{data.warmIntroPath}</div></Card>}
        </div>
      );
    ...
    case "pains":
      return (
        <div>
          {data.biggestWin && <Card style={{ borderLeft: "4px solid #10B981", background: "#F0FDF4" }}><div style={{ fontSize: 12, color: "#065F46", fontWeight: 700, marginBottom: 4 }}>🏆 FASTEST PATH TO A DEAL</div><div style={{ fontSize: 13, color: "#374151" }}>{data.biggestWin}</div></Card>}
          {data.pains?.map((p, i) => (
            <div key={i}>
          ...
          {data.pocSuggestion && <Card style={{ borderLeft: "4px solid #8B5CF6", background: "#FAF5FF" }}><div style={{ fontSize: 12, color: "#7C3AED", fontWeight: 700, marginBottom: 4 }}>💡 SUGGESTED POC (30 DAYS)</div><div style={{ fontSize: 13, color: "#374151" }}>{data.pocSuggestion}</div></Card>}
        </div>
      );
    ...
    case "competitive":
      return (
        <div>
          {data.biggestThreat && <Card style={{ background: "#FEF2F2", borderLeft: "4px solid #EF4444" }}><div style={{ fontSize: 12, color: "#991B1B", fontWeight: 700, marginBottom: 4 }}>⚠️ BIGGEST THREAT</div><div style={{ fontSize: 13, color: "#374151" }}>{data.biggestThreat}</div></Card>}
          {data.biggestAlly && <Card style={{ background: "#ECFDF5", borderLeft: "4px solid #10B981" }}><div style={{ fontSize: 12, color: "#065F46", fontWeight: 700, marginBottom: 4 }}>🤝 BEST CO-SELL ALLY</div><div style={{ fontSize: 13, color: "#374151" }}>{data.biggestAlly}</div></Card>}
                  ...
                  {week.actions?.map((a, ai) => (
                    <div key={ai} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: "1px solid #F1F5F9" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#94A3B8" }}>{a.owner} · {a.day}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div>
          {data.coldCallOpener && (
            <Card style={{ background: "#0F172A", border: "1px solid #1E293B" }}>
              <div style={{ fontSize: 11, color: "#64748B", fontWeight: 700, marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>📞 Cold Call Opener</div>
              <div style={{ fontSize: 15, color: "#F8FAFC", fontWeight: 600, lineHeight: 1.6, fontStyle: "italic" }}>"{data.coldCallOpener}"</div>
            </Card>
          )}
          {data.emails?.length > 0 && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: 1, margin: "16px 0 10px" }}>📧 Email Templates</div>
              {data.emails.map((e, i) => <EmailCard key={i} email={e} />)}
            </div>
          )}
          {data.linkedInMessages?.length > 0 && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1E40AF", textTransform: "uppercase", letterSpacing: 1, margin: "16px 0 10px" }}>🔗 LinkedIn Messages</div>
    ...
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "#F8FAFC", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "#0F172A", padding: "0 24px", borderBottom: "1px solid #1E293B", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 28, height: 28, background: "linear-gradient(135deg, #3B82F6, #8B5CF6)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🎯</div>
          <span style={{ color: "#F8FAFC", fontWeight: 800, fontSize: 15, letterSpacing: -0.3 }}>ABM Research Engine</span>
          <span style={{ color: "#475569", fontSize: 12 }}>Younion × AWS</span>
        </div>
        {running && (
        ...
        {done && <span style={{ color: "#10B981", fontSize: 13, fontWeight: 700 }}>✓ Profile complete — {company}</span>}
      </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: 260, background: "#FFFFFF", borderRight: "1px solid #E2E8F0", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          <div style={{ padding: 20, borderBottom: "1px solid #E2E8F0" }}>
            ...
            >
              {running ? "⏹ Stop Research" : "▶ Generate Profile"}
            </button>
          </div>
            ...
            {STAGES.map(stage => {
              ...
              return (
                ...
                >
                  ...
                  {status === "done" && !isActive && <span style={{ color: "#10B981", fontSize: 14 }}>✓</span>}
                  {status === "error" && <span style={{ color: "#EF4444", fontSize: 14 }}>✗</span>}
                </button>
              );
            })}
          </div>
          ...
          {running && !selectedSection && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 15, color: "#64748B" }}>Researching {company}…</div>
              </div>
            </div>
          )}
                  ...
                  <>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 13, color: "#64748B" }}>{company}{url ? ` · ${url}` : ""}</div>
                      </div>
                      {status === "running" && (
                        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 13, color: stage.color, fontWeight: 600 }}>Researching…</span>
                        </div>
                      )}
                      {status === "done" && <span style={{ marginLeft: "auto", fontSize: 13, color: "#10B981", fontWeight: 700 }}>✓ Complete</span>}
                    </div>
                    {status === "running" && !data && (
